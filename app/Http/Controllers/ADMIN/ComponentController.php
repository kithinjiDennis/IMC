...
class ComponentController extends Controller
{
    ...
    public function topmenu_create(Request $request)
    {
        ...
        $this->validate($request,[
            'title_en' => 'required|regex:/^[\pL\s\-]+$/u|min:3|max:100',
            'title_ar' => 'required|regex:/^[\pL\s\-]+$/u|min:3|max:100',
            'type' => 'required',
            'order'=>'required',
    ...
    // FOOTER 1 DATA STORE
        ...
        public function footer1_create(Request $request)
        {
            ...
            if(!empty($request->id)){
                $topmenu = Footermenu::select('order')->where('id', $request->id)->first();
                if($topmenu->order!=$request->order){
                    $sourceOrder = $topmenu->order;
                    $destinationOrder = $request->order;
                    if($sourceOrder<$destinationOrder){
                        $menusPositionToMoveDown = Footermenu::where('parent_id',$request->parent_id)->where('order','>',$sourceOrder)->where('order', '<=',$destinationOrder)->get();
                        if(!empty($menusPositionToMoveDown)){
                            foreach($menusPositionToMoveDown as $k=>$v){
                                Footermenu::where('id', $v->id)->decrement('order');
                            }
                        }
                    }
                    if($sourceOrder>$destinationOrder){
                        ...
                        if(!empty($menusPositionToMoveDown)){
                            foreach($menusPositionToMoveDown as $k=>$v){
                                Footermenu::where('id', $v->id)->increment('order');
                            }
                        }
                    }
                }
            ...
            if(empty($request->id)){
                $menusPositionToMoveDown = Footermenu::where('parent_id',$request->parent_id)->where('order', '>=',$request->order)->get();
                if(!empty($menusPositionToMoveDown)){
                    foreach($menusPositionToMoveDown as $k=>$v){
                        Footermenu::where('id', $v->id)->increment('order');
                    }
