...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
        <div class="kt-subheader__main">
            ...
            <div class="kt-subheader__group" id="kt_subheader_search">
                ...
                <form class="kt-margin-l-20" id="kt_subheader_search_form" action="{{url('/pages/listing')}}" method="GET" role="search">
        ...
        <div class="kt-subheader__toolbar">
            ...
            <a href="{{ url('pages/create') }}" class="btn btn-label-brand btn-bold">
                Add Page </a>          
        </div>
    </div>
			   ...
			   <tr class="kt-datatable__row" style="left: 0px;">
				...
				<th data-field="AgentName" class="kt-datatable__cell kt-datatable__cell--sort ">
					<span style="width: 200px;">
						<a href="{{url('pages/listing?key=name&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $pages->currentPage()}}">
							Name
				...
				<th data-field="Country" class="kt-datatable__cell kt-datatable__cell--sort">
				   <span style="width: 206px;">
						<a href="{{url('pages/listing?key=metadesc&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $pages->currentPage()}}">
							Description
							@if( session()->get( 'key' ) == 'metadesc' )					
							<span style="width: 200px;">
								<i class="{{(session()->get( 'sort_by' ) == 'asc') ? 'flaticon2-arrow-up' : 'flaticon2-arrow-down'}}"></i>
							</span>
							@endif
				...
				<th data-field="ShipDate" class="kt-datatable__cell kt-datatable__cell--sort">
					<span style="width: 206px;">
						<a href="{{url('pages/listing?key=date&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ).'&page=' . $pages->currentPage()}}">
						Date created
				   ...
				   @foreach($pages as $page)
					  ...
					  <td class="kt-datatable__cell--center kt-datatable__cell kt-datatable__cell--check" data-field="RecordID"><span style="width: 20px;"><label class="kt-checkbox kt-checkbox--single kt-checkbox--solid"><input type="checkbox" name="chkID[]" class="childBox" value="{{$page->id}}">&nbsp;<span></span></label></span></td>
						 ...
						 <td class="kt-datatable__cell--sorted kt-datatable__cell" data-field="AgentName">
							<span style="width: 200px;">
							 <span style="width: 206px;">{{ucfirst($page->name_en)}}</span>
							</span>
						 </td>
						 
						 <td data-field="Country" class="kt-datatable__cell"><span style="width: 206px;">{{$page->meta_desc_en}}</span></td>
						 <td data-field="ShipDate" class="kt-datatable__cell"><span style="width: 206px;">{{date('d M Y', $page->created_at)}}</span></td>
							...
							<span style="overflow: visible; position: relative; width: 80px;">
							   <div class="dropdown">
								  ...
								  <div class="dropdown-menu dropdown-menu-right">
									 <ul class="kt-nav">									   
										<li class="kt-nav__item"><a class="kt-nav__link" href="{{url('pages/edit', $page->id)}}"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">Edit</span></a></li>
	...
	function deleteThis(obj, id)
	{
		swal.fire({
			...
			if (result.value) {
				$('.pageloader').show();
				$.ajax({
					url: "<?php echo url('pages/delete-single-page'); ?>",
					type: 'POST',
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "<?php echo url('pages'); ?>" + '/delete-multiple-pages',
							type: 'POST',
