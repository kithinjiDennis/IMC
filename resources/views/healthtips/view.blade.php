...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
        ...
        <div class="kt-subheader__toolbar">
            ...
            <a href="{{ url('health/create') }}" class="btn btn-label-brand btn-bold">
                Add Health tips </a>
        </div>
    </div>
			   ...
			   <tr class="kt-datatable__row" style="left: 0px;">
				...
				<th data-field="AgentName" class="kt-datatable__cell kt-datatable__cell--sort ">
					<span style="width: 200px;">
						<a href="{{url('health/listing?key=name&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $pages->currentPage()}}">
							Title
				...
				<th data-field="Country" class="kt-datatable__cell kt-datatable__cell--sort">
				   <span style="width: 206px;">
						<a href="{{url('health/listing?key=description&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $pages->currentPage()}}">
							Description
							@if( session()->get( 'key' ) == 'description' )
							<span style="width: 200px;">
								<i class="{{(session()->get( 'sort_by' ) == 'asc') ? 'flaticon2-arrow-up' : 'flaticon2-arrow-down'}}"></i>
							</span>
							@endif
				...
				<th data-field="ShipDate" class="kt-datatable__cell kt-datatable__cell--sort">
					<span style="width: 206px;">
						<a href="{{url('health/listing?key=date&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ).'&page=' . $pages->currentPage()}}">
						Date created
               ...
               <tbody class="kt-datatable__body" style="">
				   @foreach($pages as $page)
					  ...
					  <td class="kt-datatable__cell--center kt-datatable__cell kt-datatable__cell--check" data-field="RecordID"><span style="width: 20px;"><label class="kt-checkbox kt-checkbox--single kt-checkbox--solid"><input type="checkbox" name="chkID[]" class="childBox" value="{{$page->id}}">&nbsp;<span></span></label></span></td>
						 ...
						 <td data-field="Country" class="kt-datatable__cell"><span style="width: 206px;">{{$page->description_en}}</span></td>
						 <td data-field="ShipDate" class="kt-datatable__cell"><span style="width: 206px;">{{$page->created_at}}</span></td>
             <td data-field="ShipDate" class="kt-datatable__cell"><span style="width: 206px;"><img width="32" height="32" src="<?php echo url('/');?>/images/healthtips/<?php echo $page->icon;?>"></img></span></td>
							...
							<span style="overflow: visible; position: relative; width: 80px;">
							   <div class="dropdown">
								  ...
								  <div class="dropdown-menu dropdown-menu-right">
									 <ul class="kt-nav">
										<li class="kt-nav__item"><a class="kt-nav__link" href="{{url('health/edit', $page->id)}}"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">Edit</span></a></li>
	...
	function deleteThis(obj, id)
	{
		swal.fire({
			...
			if (result.value) {
				$('.pageloader').show();
				$.ajax({
					url: "<?php echo url('health/delete-single-page'); ?>",
					type: 'POST',
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "<?php echo url('health'); ?>" + '/delete-multiple-pages',
							type: 'POST',
