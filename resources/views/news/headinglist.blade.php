   ...
   <div class="col-md-12">
	   ...
	   <div class="kt-portlet">
                        ...
                        Headline</h3>
										...
										<!--begin::Form-->
										<form class="kt-form" method="POST" action="{{ url('heading/create') }}" >
										@csrf
										<input type="hidden" name="id" value="<?php if(!empty($editmenu->id)){ echo $editmenu->id; }?>" >
											<div class="kt-portlet__body">
                        <div class="form-group customlink">
                          <label>Description (en)</label>
                          <textarea name="heading_en" class="form-control" aria-describedby="emailHelp" placeholder="Enter here" required><?php if(!empty($editmenu->heading_en)){ echo $editmenu->heading_en; }?></textarea>
                          <?php
                          if(isset($errors)){
                            ?>
                            <span class="invalid-feed" role="alert">
                              <strong>{{ $errors->getBag('default')->first('heading_en') }}</strong>
                            </span>
                          <?php
                          }
                           ?>
                        ...
                        <div class="form-group customlink">
                          ...
                          <textarea name="heading_ar"  class="form-control" aria-describedby="emailHelp" placeholder="Enter here" required><?php if(!empty($editmenu->heading_ar)){ echo $editmenu->heading_ar ; }?></textarea>
                          <?php
                          if(isset($errors)){
                            ?>
                            <span class="invalid-feed" role="alert">
                              <strong>{{ $errors->getBag('default')->first('heading_ar') }}</strong>
                            </span>
                          <?php
                          }
                           ?>
