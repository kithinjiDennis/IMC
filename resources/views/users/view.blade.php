...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
        <div class="kt-subheader__main">
            ...
            <div class="kt-subheader__group" id="kt_subheader_search">
                ...
                <form class="kt-margin-l-20" id="kt_subheader_search_form" action="{{url('/users/listing')}}" method="GET" role="search">
                    ...
                    <div class="dropdown" id="kt_subheader_group_actions_status_change">
                        ...
                        <div class="dropdown-menu">
                            <ul class="kt-nav">
                                ...
                                <li class="kt-nav__item" onclick="changeStatus(0)">
                                    <a href="javascrit:void(0)" class="kt-nav__link" data-toggle="status-change" data-status="3">
                                        <span class="kt-nav__link-text"><span class="kt-badge kt-badge--unified-warning kt-badge--inline kt-badge--bold">Block</span></span>
                                    </a>
                                </li>
                            </ul>
                        </div>
        ...
        <div class="kt-subheader__toolbar">
            ...
            <a href="{{ url('/users/create') }}" class="btn btn-label-brand btn-bold">
                Add User </a>          
        </div>
    </div>
            ...
            <table class="kt-datatable__table" style="display: block; min-height: 500px;">
			   ...
			   <tr class="kt-datatable__row" style="left: 0px;">
				...
				<th data-field="AgentName" class="kt-datatable__cell kt-datatable__cell--sort ">
					<span style="width: 200px;">
						<a href="{{url('users/listing?key=user&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $users->currentPage()}}">
							User
				...
				<th data-field="Country" class="kt-datatable__cell kt-datatable__cell--sort">
				   <span style="width: 206px;">
						<a href="{{url('users/listing?key=email&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $users->currentPage()}}">
							Email
							@if( session()->get( 'key' ) == 'email' )					
							<span style="width: 200px;">
								<i class="{{(session()->get( 'sort_by' ) == 'asc') ? 'flaticon2-arrow-up' : 'flaticon2-arrow-down'}}"></i>
							</span>
							@endif
				...
				<th data-field="ShipDate" class="kt-datatable__cell kt-datatable__cell--sort">
					<span style="width: 206px;">
						<a href="{{url('users/listing?key=date&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ).'&page=' . $users->currentPage()}}">
						Date created
				...
				<th data-field="Status" class="kt-datatable__cell kt-datatable__cell--sort">
					<span style="width: 100px;">
						<a href="{{url('users/listing?key=status&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $users->currentPage()}}">
						Status
						@if( session()->get( 'key' ) == 'status' )					
						<span style="width: 200px;">
							<i class="{{(session()->get( 'sort_by' ) == 'asc') ? 'flaticon2-arrow-up' : 'flaticon2-arrow-down'}}"></i>
						</span>
						@endif
               ...
               <tbody class="kt-datatable__body" style="">
				   @foreach($users as $user)
					  ...
					  <td class="kt-datatable__cell--center kt-datatable__cell kt-datatable__cell--check" data-field="RecordID"><span style="width: 20px;"><label class="kt-checkbox kt-checkbox--single kt-checkbox--solid"><input type="checkbox" name="chkID[]" class="childBox" value="{{$user->id}}">&nbsp;<span></span></label></span></td>
						 
						 <td class="kt-datatable__cell--sorted kt-datatable__cell" data-field="AgentName">
							<span style="width: 200px;">
							   <div class="kt-user-card-v2">
							      <div class="kt-user-card-v2__pic"><div class="kt-badge kt-badge--xl kt-badge--success">{{ucfirst(substr($user->name, 0, 1))}}</div></div>			
								  <span style="width: 206px;">{{ucfirst($user->name)}}</span>
							   </div>
							</span>
						 </td>
						 
						 <td data-field="Country" class="kt-datatable__cell"><span style="width: 206px;">{{$user->email}}</span></td>
						 <td data-field="ShipDate" class="kt-datatable__cell"><span style="width: 206px;">{{date('d M Y', $user->created_at)}}</span></td>
						 
						 <td data-field="Status" class="kt-datatable__cell"><span style="width: 100px;"><span onclick="blockThis(this, {{$user->id}}, {{$user->status}})" class="{{ ($user->status == 1) ? 'btn btn-bold btn-sm btn-font-sm  btn-label-success' : 'btn btn-bold btn-sm btn-font-sm  btn-label-warning'}}">{{ ($user->status == 1) ? 'Active' : 'Blocked'}}</span></span></td>
							...
							<span style="overflow: visible; position: relative; width: 80px;">
							   <div class="dropdown">
								  ...
								  <div class="dropdown-menu dropdown-menu-right">
									 <ul class="kt-nav">									   
										<li class="kt-nav__item"><a class="kt-nav__link" href="{{url('users/edit', $user->id)}}"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">Edit</span></a></li>
									 	<li onclick="deleteThis(this, {{$user->id}})" class="kt-nav__item delete_user"><a class="kt-nav__link delete_user" href="javascript:void(0);"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text delete_user">Delete</span>
									 	<!-- <li onclick="blockThis(this, {{$user->id}}, {{$user->status}})" class="kt-nav__item"><a class="kt-nav__link" href="javascript:viod(0);"><i class="kt-nav__link-icon flaticon2-expand"></i><span class="kt-nav__link-text">{{($user->status==1) ? 'Block': 'Un-block'}}</span></a></li> -->
									 </ul>
								  </div>
							   </div>
							</span>
            ...
            <div class="kt-datatable__pager kt-datatable--paging-loaded">
				...
				<!-- <div class="kt-datatable__pager-info">
					<div class="dropdown bootstrap-select kt-datatable__pager-size" style="width: 60px;">
						<select  onchange="getDropdownval(this);" class="selectpicker kt-datatable__pager-size" title="Select page size" data-width="60px" data-selected="10" tabindex="-98">
