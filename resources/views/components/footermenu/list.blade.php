...
Footer 1	<?php if($submenu==1){
            ...
            <div class="kt-subheader__group" id="kt_subheader_search">
                ...
                <!-- <form class="kt-margin-l-20" id="kt_subheader_search_form" action="{{url('/admin/footer1/list')}}" method="GET" role="search">
        ...
        <div class="kt-subheader__toolbar">
            ...
            <a href="{{ url('admin/footer1/list') }}" class="btn btn-label-brand btn-bold">
				Add Menu </a>
				<?php  }elseif(!empty($editmenu->id) && $submenu==1){ ?>
					<a href="<?php echo url('admin/footer1/list/'.$parentId.'/submenu'); ?>" class="btn btn-label-brand btn-bold">
						Add Menu </a>
						<?php  }
				?>
        </div>
         ...
         <div class="kt-datatable kt-datatable--default kt-datatable--brand kt-datatable--loaded" id="kt_apps_user_list_datatable" style="">
            <table class="kt-datatable__table" style="display: block;">
			   ...
			   <tr class="kt-datatable__row" style="left: 0px;">
				...
				<th data-field="MenuTitleEn" class="kt-datatable__cell kt-datatable__cell--sort " style="background-color:#ecebeb;text-align: center;width:{{$width}}%;padding-left: 0;">
					<span>
					<a href="javascript:;">
							<b>Title(EN)</b>
						</a>
				...
				<th data-field="MenuTitleAr" class="kt-datatable__cell kt-datatable__cell--sort " style="background-color:#ecebeb;text-align: center;width:{{$width}}%;">
					<span>
						<a href="javascript:;">
								<b>Title(AR)</b>
						</a>
				...
				<th data-field="MenuLink" class="kt-datatable__cell kt-datatable__cell--sort "  style="background-color:#ecebeb;text-align: center;width:{{$width}}%;">
					<span >
						<a href="javascript:;">
							<b>	Link</b>
						</a>
				...
				<?php } ?>
				<th  style="background-color:#ecebeb;text-align: center;width:{{$width}}%;" data-field="Actions" data-autohide-disabled="false" class="kt-datatable__cell kt-datatable__cell--sort"><span><a href="javascript:;">
          <b>Actions</span></th>
				</tr>
               </thead>
							   ...
							   <div class="dropdown">
								  ...
								  <div class="dropdown-menu dropdown-menu-right">
									 <ul class="kt-nav">
										...
										<?php if($submenu==1){ ?>
											<a class="kt-nav__link" href="<?php echo url('admin/footer1/list/'.$parentId.'/submenu/'.$menu->id); ?>">
										<?php }else{ ?>
											<a class="kt-nav__link" href="{{url('admin/footer1/list', $menu->id)}}">
										<?php } ?>
	   ...
	   <div class="kt-portlet">
										<div class="kt-portlet__head">
											<div class="kt-portlet__head-label">
												...
												<?php if(!empty($editmenu->id)){ echo 'Edit Menu'; }else{ echo 'Add Footer Menu'; }?>
												</h3>
											</div>
										</div>

										<!--begin::Form-->
										<form class="kt-form" method="POST" enctype="multipart/form-data" action="{{ url('admin/footer1/create') }}" >
										@csrf
...
<script>
	...
	function deleteThis(obj, id)
	{
		swal.fire({
			...
			if (result.value) {
				$('.pageloader').show();
				$.ajax({
					url: "<?php echo url('admin/footer1/delete'); ?>",
					type: 'POST',
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "<?php echo url('footer1'); ?>" + '/delete-multiple-menu',
							type: 'POST',
	...
	function updateOrder(selectedData) {
		$('.pageloader').show();
		$.ajax({
			url: "<?php echo url('admin/footer1'); ?>" + '/update-menu-order',
			type: 'POST',
	...
	function FilterMenuList(selectedData) {
		$('.pageloader').show();
		$.ajax({
			url: "<?php echo url('admin/footer1'); ?>" + '/list?q=',
			type: 'POST',
